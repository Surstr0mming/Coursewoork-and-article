...
def display_text():
    ...
    if len(entered_text) < 1:
        # Error message if text length is less than 1
        error_label.config(text="The text is not long enough", fg="black")
    elif crypt(entered_text) == 'null':
        for char in entered_text:
            if char not in alphabet:
                if char not in not_in_alphabet:
                    not_in_alphabet.append(char)
        error_label.config(text=f"Text contains character(s), {not_in_alphabet}, which are not in the dictionary", fg="black")
    else:
        global_text = entered_text
...
def show_error_window():
    ...
    error_label = tk.Label(error_window, text="An error occurred, please restart the program", padx=20, pady=20)
    error_label.pack()
...
if __name__ == '__main__':
    ...
    window = tk.Tk()
    window.title("Choosing the type of work")

    # Adding an explanation of the type of work
    label = tk.Label(window, text="Select the type of work:")
    label.pack()

    # Creation of selection options
    job_type_var = tk.StringVar()
    job_type_var.set('1')  # By default, we set the first type of operation

    # Radio buttons for selecting the type of operation
    radio_button1 = tk.Radiobutton(window, text="encryption", variable=job_type_var, value='1')
    radio_button2 = tk.Radiobutton(window, text="decoding", variable=job_type_var, value='2')

    radio_button1.pack()
    radio_button2.pack()

    # Confirm button
    submit_button = tk.Button(window, text="CONFIRM", command=submit)
    submit_button.pack()
    window.mainloop()
    ...
    if type_of_work == '1':
        ...
        # Create a button to output the entered text
        button = tk.Button(root, text="Display the text", command=display_text)
        button.pack()
        ...
        window = tk.Tk()
        window.title("Introduction of modules of the first order")

        # Creating and placing elements in the window
        label1 = ttk.Label(window, text="p1:")
        label1.grid(row=0, column=0, padx=10, pady=10)
        entry1 = ttk.Entry(window, width=60)  # We set the width to 20 characters
        entry1.grid(row=0, column=1, padx=10, pady=10)

        label2 = ttk.Label(window, text="p2:")
        label2.grid(row=1, column=0, padx=10, pady=10)
        entry2 = ttk.Entry(window, width=60)  # We set the width to 20 characters
        entry2.grid(row=1, column=1, padx=10, pady=10)

        label3 = ttk.Label(window, text="p3:")
        label3.grid(row=2, column=0, padx=10, pady=10)
        entry3 = ttk.Entry(window, width=60)  # We set the width to 20 characters
        entry3.grid(row=2, column=1, padx=10, pady=10)

        calculate_button = ttk.Button(window, text="set modules", command=check_p)
        ...
        window = tk.Tk()
        window.title("Introduction of modules of the second order")
        ...
        calculate_button = ttk.Button(window, text="set modules", command=check_q_1_3)
        calculate_button.grid(row=3, column=0, columnspan=2, pady=10)
        ...
        calculate_button = ttk.Button(window, text="set modules", command=check_q_4_6)
        calculate_button.grid(row=3, column=0, columnspan=2, pady=10)
        ...
        calculate_button = ttk.Button(window, text="set modules", command=check_q_7_9)
        calculate_button.grid(row=3, column=0, columnspan=2, pady=10)
        ...
        # Creating the main window
        root = tk.Tk()
        root.title("encrypted text in windowed mode")
       ...
       # Set the window size
        ...
        # Button to turn full screen mode on and off
        fullscreen_button = tk.Button(root, text="Toggle Fullscreen", command=toggle_fullscreen)
        fullscreen_button.pack()

        # Button for copying selected text
        copy_button = tk.Button(root, text="Copy", command=copy_text)
        copy_button.pack()

       # Start the main event loop
        root.mainloop()
        print(f"Encrypting time for {len(global_text)} lengths text is for b1-b3 is{b1_3_time} and for b11-b39 is{b11_39_time}")
        ...
        root = tk.Tk()
        root.title("Decrypted text in windowed mode")
       ...
       # Creating a scrollable text field
        ...
        print(f"Decoding time for {len(cryptet_text_screen)} lengths text is for b1-b3 is{b1_3_time} and for b11-b39 is{b11_39_time}")
    else:
